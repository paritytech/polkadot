...
fn it_works() {
	...
	assert!(MockXcmSender::send_xcm(Null, Xcm::WithdrawAsset {
		assets: vec![], effects: vec![]
	}).is_ok());
	assert_eq!(sent_xcm(), vec![(Null, Xcm::WithdrawAsset {
		assets: vec![], effects: vec![]
	})]);
}
...
fn executor_works() {
	use xcm::v0::Xcm;
	let r = XcmExecutor::<XcmConfig>::execute_xcm(Null, Xcm::WithdrawAsset {
		assets: vec![], effects: vec![]
	}, 1000);
	assert_eq!(r, Outcome::Complete(1000));
}
