...
pub mod pallet {
	...
	pub(super) type Queries<T: Config> =
		StorageMap<_, Blake2_128Concat, QueryId, QueryStatus<frame_system::BlockNumberOf<T>>, OptionQuery>;
	...
	pub mod migrations {
		...
		pub fn migrate_to_v1<T: Config, P: GetStorageVersion + PalletInfoAccess>(
			...
			if on_chain_storage_version < 1 {
				let mut count = 0;
				Queries::<T>::translate::<QueryStatusV0<frame_system::BlockNumberOf<T>>, _>(|_key, value| {
					count += 1;
					Some(value.into())
				});
...
impl<T: Config> QueryHandler for Pallet<T> {
	...
	fn new_query(
		...
		timeout: frame_system::BlockNumberOf<T>,
