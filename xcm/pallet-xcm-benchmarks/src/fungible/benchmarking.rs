...
use super::*;
use crate::{
	account_and_location, account_id_junction, execute_xcm, worst_case_holding, AssetTransactorOf,
	XcmCallOf,
};
...
benchmarks_instance_pallet! {
	...
	deposit_asset {
		...
		assert_ok!(execute_xcm::<T>(origin, order));
	} verify {
	...
	// 	};
	// }: {
	// 	assert_ok!(execute_xcm::<T>(origin, holding, order));
	// } verify {
	// 	// dest should have received some asset.
	// 	assert!(!T::TransactAsset::balance(&dest_account).is_zero())
	// }
	...
	// 	assert_ok!(execute_xcm::<T>(origin, holding, order));
	// } verify {
	// 	// execute_xcm succeeding is enough to verify this completed.
	...
	// 	let instruction = Instruction::WithdrawAsset::<XcmCallOf<T>> { assets: vec![asset].into(), effects: vec![] };
	// }: {
	...
	// 	assert!(T::TransactAsset::balance(&ben_account).is_zero());
	// 	let instruction = Instruction::TransferAsset { assets, beneficiary: ben_location };
	// 	let xcm = Xcm(vec![instruction]);
	// }: {
	// 	assert_ok!(execute_xcm::<T>(sender_location, Default::default(), xcm).ensure_complete());
	// } verify {
	...
	// 	let instruction = Instruction::TransferReserveAsset { assets, dest: dest_location, effects };
	// }: {
	// 	assert_ok!(execute_xcm::<T>(sender_location, Default::default(), xcm).ensure_complete());
	// } verify {
	// 	assert!(T::TransactAsset::balance(&sender_account).is_zero());
