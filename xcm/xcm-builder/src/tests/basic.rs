...
fn code_registers_should_work() {
	...
	assert_eq!(limit, Weight::from_ref_time(70).set_proof_size(DEFAULT_PROOF_SIZE));

	let hash = fake_message_hash(&message);

	let r = XcmExecutor::<TestConfig>::execute_xcm(Here, message.clone(), hash, limit);
	assert_eq!(r, Outcome::Complete(Weight::from_ref_time(50))); // We don't pay the 20 weight for the error handler.
	...
	assert_eq!(r, Outcome::Complete(Weight::from_ref_time(70).set_proof_size(DEFAULT_PROOF_SIZE))); // We pay the full weight here.
