...
impl <Call> ::scale_info::TypeInfo for Xcm<Call> where
Call: ::scale_info::TypeInfo + 'static,
// Vec<Order<Call>>: ::scale_info::TypeInfo + 'static,
// Vec<Order<Call>>: ::scale_info::TypeInfo + 'static,
// Vec<Order<Call>>: ::scale_info::TypeInfo + 'static,
DoubleEncoded<Call>: ::scale_info::TypeInfo + 'static,
Call: ::scale_info::TypeInfo + 'static {
	type Identity = Self;
	fn type_info() -> ::scale_info::Type {
		::scale_info::Type::builder().path(::scale_info::Path::new("Xcm",
				"xcm::v0")).type_params(<[_]>::into_vec(scale_info::prelude::boxed::Box::new(
				[::scale_info::meta_type::<Call>()]))).docs(&[" Cross-Consensus Message: A message from one consensus system to another.",
			"",
			" Consensus systems that may send and receive messages include blockchains and smart contracts.",
			"",
			" All messages are delivered from a known *origin*, expressed as a `MultiLocation`.",
			"",
			" This is the inner XCM format and is version-sensitive. Messages are typically passed using the outer",
			" XCM format, known as `VersionedXcm`."]).variant(::scale_info::build::Variants::with_fields().variant("WithdrawAsset",
				::scale_info::build::Fields::named().field_of::<Vec<MultiAsset>>("assets",
					"Vec<MultiAsset>",
					&[]).field_of::<Vec<Order<Call>>>("effects",
					"Vec<Order<Call>>",
					&[]),
					&[" Withdraw asset(s) (`assets`) from the ownership of `origin` and place them into `holding`. Execute the",
					" orders (`effects`).",
					...
					" Kind: *Instruction*.",
					"",
					" Errors:"]).variant("ReserveAssetDeposit",
					::scale_info::build::Fields::named().field_of::<Vec<MultiAsset>>("assets",
						"Vec<MultiAsset>",
						&[]).field_of::<Vec<Order<Call>>>("effects",
						"Vec<Order<Call>>",
						...
						::scale_info::build::Fields::named().field_of::<Vec<MultiAsset>>("assets",
							...
							" Kind: *Trusted Indication*.",
							"",
							" Errors:"]).variant("QueryResponse",
							::scale_info::build::Fields::named().compact_of::<u64>("query_id",
								"u64",
								&[]).field_of::<Response>("response",
								"Response",
								&[]),
								&[" Indication of the contents of the holding account corresponding to the `QueryHolding` order of `query_id`.",
								"",
								...
								" Kind: *Information*.",
								"",
								" Errors:"]).variant("TransferAsset",
								::scale_info::build::Fields::named().field_of::<Vec<MultiAsset>>("assets",
									"Vec<MultiAsset>",
									&[]).field_of::<MultiLocation>("dest",
									"MultiLocation",
									&[]),
									&[" Withdraw asset(s) (`assets`) from the ownership of `origin` and place equivalent assets under the",
									" ownership of `dest` within this consensus system.",
									...
									" Kind: *Instruction*.",
									"",
									" Errors:"]).variant("TransferReserveAsset",
									::scale_info::build::Fields::named().field_of::<Vec<MultiAsset>>("assets",
										"Vec<MultiAsset>",
										&[]).field_of::<MultiLocation>("dest",
										"MultiLocation",
										&[]).field_of::<Vec<Order<()>>>("effects",
										"Vec<Order<()>>",
										&[]),
										&[" Withdraw asset(s) (`assets`) from the ownership of `origin` and place equivalent assets under the",
										" ownership of `dest` within this consensus system.",
										...
										" Kind: *Instruction*.",
										"",
										" Errors:"]).variant("Transact",
										::scale_info::build::Fields::named().field_of::<OriginKind>("origin_type",
											"OriginKind",
											&[]).field_of::<u64>("require_weight_at_most",
											"u64",
											&[]).field_of::<DoubleEncoded<Call>>("call",
											"DoubleEncoded<Call>",
											&[]),
											&[" Apply the encoded transaction `call`, whose dispatch-origin should be `origin` as expressed by the kind",
											" of origin `origin_type`.",
											...
											" Kind: *Instruction*.",
											"",
											" Errors:"]).variant("HrmpNewChannelOpenRequest",
											::scale_info::build::Fields::named().compact_of::<u32>("sender",
												"u32",
												&[]).compact_of::<u32>("max_message_size",
												"u32",
												&[]).compact_of::<u32>("max_capacity",
												"u32",
												&[]),
												&[" A message to notify about a new incoming HRMP channel. This message is meant to be sent by the",
												" relay-chain to a para.",
												...
												" Safety: The message should originate directly from the relay-chain.",
												"",
												" Kind: *System Notification*"]).variant("HrmpChannelAccepted",
												::scale_info::build::Fields::named().compact_of::<u32>("recipient",
													"u32",
													&[]),
													&[" A message to notify about that a previously sent open channel request has been accepted by",
													" the recipient. That means that the channel will be opened during the next relay-chain session",
													...
													" Kind: *System Notification*",
													"",
													" Errors:"]).variant("HrmpChannelClosing",
													::scale_info::build::Fields::named().compact_of::<u32>("initiator",
														"u32",
														&[]).compact_of::<u32>("sender",
														"u32",
														&[]).compact_of::<u32>("recipient",
														"u32",
														&[]),
														&[" A message to notify that the other party in an open channel decided to close it. In particular,",
														" `inititator` is going to close the channel opened from `sender` to the `recipient`. The close",
														...
														" Kind: *System Notification*",
														"",
														" Errors:"]).variant("RelayedFrom",
														::scale_info::build::Fields::named().field_of::<MultiLocation>("who",
															"MultiLocation",
															&[]).field_of::<alloc::boxed::Box<Xcm<Call>>>("message",
															"alloc::boxed::Box<Xcm<Call>>",
															&[]),
															&[" A message to indicate that the embedded XCM is actually arriving on behalf of some consensus",
															" location within the origin.",
															"",
