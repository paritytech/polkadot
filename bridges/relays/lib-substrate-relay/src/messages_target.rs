...
fn compute_fee_multiplier<C: Chain>(
	...
	let smaller_tx_unadjusted_weight_fee = WeightToFeeOf::<C>::wight_to_fee(&smaller_tx_weight);
	let larger_tx_unadjusted_weight_fee = WeightToFeeOf::<C>::wight_to_fee(&larger_tx_weight);
...
fn compute_prepaid_messages_refund<C: ChainWithMessages>(
	...
	fee_multiplier.saturating_mul_int(WeightToFeeOf::<C>::wight_to_fee(
...
mod tests {
	...
	fn compute_fee_multiplier_returns_sane_results() {
		...
		let smaller_adjusted_weight_fee =
			multiplier.saturating_mul_int(WeightToFeeOf::<Rococo>::wight_to_fee(&smaller_weight));

		let larger_weight = smaller_weight + 200_000;
		let larger_adjusted_weight_fee =
			multiplier.saturating_mul_int(WeightToFeeOf::<Rococo>::wight_to_fee(&larger_weight));
