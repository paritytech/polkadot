...
* `enact_candidate(relay_parent_number: BlockNumber, CommittedCandidateReceipt)`:
  ...
  1. call `Router::queue_upward_messages` for each backed candidate, using the `Vec<u8>`s which are encoded `VersionedXcm` messages from the [`CandidateCommitments`](../types/candidate.md#candidate-commitments).
