...
* `process_bitfields(Bitfields, core_lookup: Fn(CoreIndex) -> Option<ParaId>)`:
  ...
  1. Pass information about all cores to the [`Approvals`](approvals.md) module via the `approve_included` routine. For all cores that have just been freed, provide the candidate descriptor, the index of the backing group, and the `availability_votes` bitfield.
