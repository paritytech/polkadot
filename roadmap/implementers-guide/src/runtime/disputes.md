...
```rust
decl_storage! {
	trait Store for Module<T: Config> as Disputes {
		/// Set of candidates disputed on this fork.
		Disputed get(fn disputed_candidates): Vec<CandidateHash>;

		/// Resolved candidate hashes, with associated timestamps when each of them was resolved.
		Resolved: map hasher(blake2_128_concat) CandidateHash => Timestamp;

		/// Access older block data.
		BlockHistory get(fn block_history): Vec<<Block as BlockT>::Hash>;
	}
}
```
...
1. `UnsignedTransaction`/`Extrinsic` from node with `Event`
