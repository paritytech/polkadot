...
if [ ! -x "$polkadot" ] || [ "$polkadot" -ot "$last_modified_rust_file" ]; then
  cargo build --release --features="runtime-metrics"
fi
...
function run_node() {
  ...
  local stdout
  local stderr
  mkdir -p $HOME/node/$name/
  stdout="$HOME/node/$name/stdout"
  stderr="$HOME/node/$name/stderr"
  touch $stdout
  touch $stderr
  node_pipes+=("$stdout")
  node_pipes+=("$stderr")
  ...
  local port=$((30335+node_offset))
  local rpc_port=$((9935+node_offset))
  local ws_port=$((9946+node_offset))
  local prometheus_port=$((30000+node_offset))
  node_offset=$((node_offset+1))

  # start the node
  RUST_BACKTRACE=full "$polkadot" \
    -l runtime=trace \
    --chain "$chainspec" \
    --port "$port" \
    --base-path ~/node/$name/ \
    ...
    --tracing-targets wasm_tracing=trace,runtime=trace\
    "$(flagify "$name")" \
  > "$stdout" \
  2> "$stderr" \
  &
