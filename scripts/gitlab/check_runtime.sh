...
check_runtime () {
	...
	add_spec_version="$(git diff origin/master...${CI_COMMIT_SHA} ${1} \
		| sed -n -r "s/^\+[[:space:]]+spec_version: +([0-9]+),$/\1/p")"
	sub_spec_version="$(git diff origin/master...${CI_COMMIT_SHA} ${1} \
		| sed -n -r "s/^\-[[:space:]]+spec_version: +([0-9]+),$/\1/p")"
	...
	then
		...
		add_impl_version="$(git diff origin/master...${CI_COMMIT_SHA} ${1} \
			| sed -n -r 's/^\+[[:space:]]+impl_version: +([0-9]+),$/\1/p')"
		sub_impl_version="$(git diff origin/master...${CI_COMMIT_SHA} ${1} \
			| sed -n -r 's/^\-[[:space:]]+impl_version: +([0-9]+),$/\1/p')"
