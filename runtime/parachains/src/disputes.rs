...
impl<T: Config> DisputesHandler<frame_system::BlockNumberOf<T>> for pallet::Pallet<T>
where
	frame_system::BlockNumberOf<T>: Ord,
{
	fn is_frozen() -> bool {
	...
	fn filter_dispute_data(
		...
		post_conclusion_acceptance_period: frame_system::BlockNumberOf<T>,
	) -> Option<CheckedDisputeStatementSet> {
	...
	fn note_included(
		session: SessionIndex,
		candidate_hash: CandidateHash,
		included_in: frame_system::BlockNumberOf<T>,
	) {
	...
	fn initializer_on_new_session(notification: &SessionChangeNotification<frame_system::BlockNumberOf<T>>) {
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config + configuration::Config + session_info::Config {
		...
		type SlashingHandler: SlashingHandler<frame_system::BlockNumberOf<Self>>;
	...
	pub(super) type Disputes<T: Config> = StorageDoubleMap<
		...
		Blake2_128Concat,
		CandidateHash,
		DisputeState<frame_system::BlockNumberOf<T>>,
	>;
	...
	pub(super) type Frozen<T: Config> = StorageValue<_, Option<frame_system::BlockNumberOf<T>>, ValueQuery>;
	...
	pub enum Event<T: Config> {
		...
		Revert(frame_system::BlockNumberOf<T>),
	}
...
impl<T: Config> Pallet<T> {
	...
	fn process_checked_dispute_data(
		...
		dispute_post_conclusion_acceptance_period: frame_system::BlockNumberOf<T>,
	...
	#[allow(unused)]
	pub(crate) fn disputes() -> Vec<(SessionIndex, CandidateHash, DisputeState<frame_system::BlockNumberOf<T>>)> {
	...
	pub(crate) fn revert_and_freeze(revert_to: frame_system::BlockNumberOf<T>) {
