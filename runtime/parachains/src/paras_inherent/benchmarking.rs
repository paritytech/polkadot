...
use primitives::v1::PARACHAINS_INHERENT_IDENTIFIER;
...
benchmarks! {
	...
	create_inherent {
		...
		let dispute_sessions = vec![2; b as usize];
		let cores_with_backed: BTreeMap<_, _> = {
			let mut votes = Vec::new();
			for i in 0..=a {
				votes.push((i, BenchBuilder::<T>::fallback_min_validity_votes()))
			}
		...
		let scenario = BenchBuilder::<T>::new()
			.build(cores_with_backed, &dispute_sessions, None);
		...
		assert!(Pallet::<T>::create_inherent(&inherent_data).is_some());
	}
}
