...
benchmarks! {
	force_set_current_code {
		...
		<Pallet<T> as Store>::CurrentCodeHash::insert(&para_id, new_code.clone());
	...
	force_note_new_head {
		...
		let para_id = ParaId::from(1000);
		let new_head = HeadData(vec![0; s as usize]);
		let old_code = ValidationCode(vec![0]);
		<Pallet<T> as Store>::CurrentCodeHash::insert(&para_id, old_code);
