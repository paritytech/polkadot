...
pub mod pallet {
	...
	pub(super) type PvfActiveVoteMap<T: Config> = StorageMap<
		...
		Twox64Concat,
		ValidationCodeHash,
		PvfCheckActiveVoteState<frame_system::BlockNumberOf<T>>,
		OptionQuery,
	>;
	...
	pub(super) type PastCodeHash<T: Config> =
		StorageMap<_, Twox64Concat, (ParaId, frame_system::BlockNumberOf<T>), ValidationCodeHash>;
	...
	pub(super) type PastCodeMeta<T: Config> =
		StorageMap<_, Twox64Concat, ParaId, ParaPastCodeMeta<frame_system::BlockNumberOf<T>>, ValueQuery>;
	...
	pub(super) type PastCodePruning<T: Config> =
		StorageValue<_, Vec<(ParaId, frame_system::BlockNumberOf<T>)>, ValueQuery>;
	...
	pub(super) type FutureCodeUpgrades<T: Config> =
		StorageMap<_, Twox64Concat, ParaId, frame_system::BlockNumberOf<T>>;
...
impl<T: Config> Pallet<T> {
	...
	pub(crate) fn initializer_initialize(now: frame_system::BlockNumberOf<T>) -> Weight {
	...
	pub(crate) fn initializer_finalize(now: frame_system::BlockNumberOf<T>) {
	...
	// that are too old.
	fn prune_old_code(now: frame_system::BlockNumberOf<T>) -> Weight {
		let config = configuration::Pallet::<T>::config();
		...
		let pruning_tasks_done =
			PastCodePruning::<T>::mutate(|pruning_tasks: &mut Vec<(_, frame_system::BlockNumberOf<T>)>| {
	...
	fn process_scheduled_upgrade_changes(now: frame_system::BlockNumberOf<T>) -> Weight {
		// account weight for `UpcomingUpgrades::mutate`.
		let mut weight = T::DbWeight::get().reads_writes(1, 1);
		let upgrades_signaled = UpcomingUpgrades::<T>::mutate(
			|upcoming_upgrades: &mut Vec<(ParaId, frame_system::BlockNumberOf<T>)>| {
	...
	fn process_scheduled_upgrade_cooldowns(now: frame_system::BlockNumberOf<T>) {
		UpgradeCooldowns::<T>::mutate(|upgrade_cooldowns: &mut Vec<(ParaId, frame_system::BlockNumberOf<T>)>| {
	...
	fn groom_ongoing_pvf_votes(
		cfg: &configuration::HostConfiguration<frame_system::BlockNumberOf<T>>,
	...
	fn enact_pvf_accepted(
		...
		causes: &[PvfCheckCause<frame_system::BlockNumberOf<T>>],
	...
	fn enact_pvf_rejected(
		code_hash: &ValidationCodeHash,
		causes: Vec<PvfCheckCause<frame_system::BlockNumberOf<T>>>,
	) -> Weight {
		let mut weight = Weight::zero();
	...
	fn kick_off_pvf_check(
		cause: PvfCheckCause<frame_system::BlockNumberOf<T>>,
		code_hash: ValidationCodeHash,
		code: ValidationCode,
		cfg: &configuration::HostConfiguration<frame_system::BlockNumberOf<T>>,
	) -> Weight {
	...
	pub(crate) fn note_new_head(
		id: ParaId,
		new_head: HeadData,
		execution_context: frame_system::BlockNumberOf<T>,
	) -> Weight {
