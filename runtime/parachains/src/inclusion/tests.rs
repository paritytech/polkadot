	...
	new_test_ext(genesis_config(paras)).execute_with(|| {
		...
		<PendingAvailability<Test>>::insert(
			...
			CandidatePendingAvailability {
				...
				backing_group: GroupIndex::from(0_usize),
			},
		);
		...
		<PendingAvailability<Test>>::insert(
			...
			CandidatePendingAvailability {
				...
				backed_in_number: 0,
				backers: default_backing_bitfield(),
				backing_group: GroupIndex::from(1_usize),
			},
		);
	...
	new_test_ext(genesis_config(paras)).execute_with(|| {
		...
		let group_validators = |group_index: GroupIndex| {
			match group_index {
				...
				group_index if group_index == GroupIndex::from(2_usize) => Some(vec![4]),
		...
		let chain_a_assignment = CoreAssignment {
			...
			group_idx: GroupIndex::from(0_usize),
		};
		...
		let chain_b_assignment = CoreAssignment {
			...
			para_id: chain_b,
			kind: AssignmentKind::Parachain,
			group_idx: GroupIndex::from(1_usize),
		};
		...
		let thread_a_assignment = CoreAssignment {
			...
			group_idx: GroupIndex::from(2_usize),
		};
		...
		// unscheduled candidate.
		{
			...
			let backed = block_on(back_candidate(
				candidate,
				&validators,
				group_validators(GroupIndex::from(0_usize)).unwrap().as_ref(),
				&keystore,
				&signing_context,
		...
		// candidates out of order.
		{
			...
			let backed_b = block_on(back_candidate(
				candidate_b,
				&validators,
				group_validators(GroupIndex::from(1_usize)).unwrap().as_ref(),
				&keystore,
				&signing_context,
				BackingKind::Threshold,
			));
		...
		// candidate has wrong collator.
		{
			...
			let backed = block_on(back_candidate(
				candidate,
				&validators,
				group_validators(GroupIndex::from(2_usize)).unwrap().as_ref(),
				&keystore,
				&signing_context,
				BackingKind::Threshold,
			));
	...
	new_test_ext(genesis_config(paras)).execute_with(|| {
		...
		assert_eq!(
			...
			Some(CandidatePendingAvailability {
				...
				backing_group: GroupIndex::from(2_usize),
			})
		);
		assert_eq!(
...
fn can_include_candidate_with_ok_code_upgrade() {
	...
	new_test_ext(genesis_config(paras)).execute_with(|| {
		...
		let group_validators = |group_index: GroupIndex| {
			match group_index {
				group_index if group_index == GroupIndex::from(0_usize) =>
					Some(vec![0, 1, 2, 3, 4]),
