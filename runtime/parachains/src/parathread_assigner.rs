...
use primitives::{CollatorId, CoreIndex, Id as ParaId};
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		/// The maximum number of orders stored by the price controller.
		#[pallet::constant]
	...
	// TODO naming is hard
	#[pallet::storage]
	pub type SpotQueue<T> = StorageValue<_, BoundedVec<SpotQueueClaim, <T as Config>::MaxOrders>>;
...
impl<T: Config> Pallet<T> {
	...
	pub(crate) fn initializer_initialize(now: T::BlockNumber) -> Weight {
		// TODO calculate spot price multiplier and store it.
		Weight::zero()
	}
	...
	fn on_new_sesssion(_n_lookahead: u32) {}
	...
	///
	/// Returns:
	/// - An `Option<FixedI64>` in the range of `1.0`- `FixedI64::MAX`
	/// Parameters:
	...
	pub(crate) fn calculate_spot_price(
		...
		if queue_capacity > 0 {
			// (queue_size / queue_capacity) - target_queue_utilization
			let queue_util_diff = FixedI64::from_rational(queue_size.into(), queue_capacity.into())
				.sub(target_queue_utilization.into());
			// variability * queue_util_diff
			if let Some(var_times_qud) = queue_util_diff.const_checked_mul(variability.into()) {
				// variability^2 * queue_util_diff^2
				if let Some(var_times_qud_pow) = var_times_qud.const_checked_mul(var_times_qud) {
					// (variability^2 * queue_util_diff^2)/2
					if let Some(div_by_two) = var_times_qud_pow.const_checked_div(2.into()) {
						// traffic * (1 + queue_util_diff) + div_by_two
						match traffic.const_checked_mul(var_times_qud.add(1.into()) + div_by_two) {
							Some(t) => {
