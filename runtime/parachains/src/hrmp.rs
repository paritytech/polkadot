...
pub mod pallet {
	...
	#[pallet::storage]
	pub type HrmpWatermarks<T: Config> = StorageMap<_, Twox64Concat, ParaId, frame_system::BlockNumberOf<T>>;
	...
	pub type HrmpChannelContents<T: Config> = StorageMap<
		...
		Twox64Concat,
		HrmpChannelId,
		Vec<InboundHrmpMessage<frame_system::BlockNumberOf<T>>>,
		ValueQuery,
	>;
	...
	pub type HrmpChannelDigests<T: Config> =
		StorageMap<_, Twox64Concat, ParaId, Vec<(frame_system::BlockNumberOf<T>, Vec<ParaId>)>, ValueQuery>;
...
impl<T: Config> Pallet<T> {
	...
	pub(crate) fn initializer_on_new_session(
		notification: &initializer::SessionChangeNotification<frame_system::BlockNumberOf<T>>,
	...
	fn process_hrmp_open_channel_requests(config: &HostConfiguration<frame_system::BlockNumberOf<T>>) {
	...
	pub(crate) fn check_hrmp_watermark(
		recipient: ParaId,
		relay_chain_parent_number: frame_system::BlockNumberOf<T>,
		new_hrmp_watermark: frame_system::BlockNumberOf<T>,
	) -> Result<(), HrmpWatermarkAcceptanceErr<frame_system::BlockNumberOf<T>>> {
	...
	pub(crate) fn prune_hrmp(recipient: ParaId, new_hrmp_watermark: frame_system::BlockNumberOf<T>) -> Weight {
	...
	pub(crate) fn inbound_hrmp_channels_contents(
		recipient: ParaId,
	) -> BTreeMap<ParaId, Vec<InboundHrmpMessage<frame_system::BlockNumberOf<T>>>> {
		let sender_set = HrmpIngressChannelsIndex::<T>::get(&recipient);
