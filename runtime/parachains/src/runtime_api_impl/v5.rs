...
) -> (Vec<Vec<ValidatorIndex>>, GroupRotationInfo<frame_system::BlockNumberOf<T>>) {
...
pub fn availability_cores<T: initializer::Config>() -> Vec<CoreState<T::Hash, frame_system::BlockNumberOf<T>>> {
...
) -> (frame_system::BlockNumberOf<T>, <T as frame_system::Config>::Hash) {
...
) -> Option<(PersistedValidationData<T::Hash, frame_system::BlockNumberOf<T>>, ValidationCodeHash)> {
	let (relay_parent_number, relay_parent_storage_root) = current_relay_parent::<T>();
...
) -> Vec<(SessionIndex, CandidateHash, DisputeState<frame_system::BlockNumberOf<T>>)> {
