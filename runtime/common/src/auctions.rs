...
decl_storage! {
	trait Store for Module<T: Config> as Auctions {
		...
		pub ReservedAmounts get(fn reserved_amounts):
			map hasher(blake2_128_concat) (T::AccountId, ParaId) => Option<BalanceOf<T>>;

		/// The winning bids for each of the 10 ranges at each sample in the final Ending Period of
		/// the current auction. The map's key is the 0-based index into the Sample Size. The
		/// first sample of the ending period is 0; the last is `Sample Size - 1`.
		pub Winning get(fn winning): map hasher(blake2_128_concat) T::BlockNumber => Option<WinningData<T>>;
	}
}
