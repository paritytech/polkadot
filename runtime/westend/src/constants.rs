...
pub mod currency {
	...
	/// The number of balance UNITS per one WND. 1x10^12
	pub const UNITS_PER_WND: Balance = 1_000_000_000_000;
	/// Easier to reference this way.
	pub const WND: Balance = UNITS_PER_WND;

	/// WND has no USD value, so we simply say 1 WND is 1 USD for these configurations.
	/// NOTE: This is written funny to more easily interpret the value of 1 USD per WND.
	pub const MILLICENTS_PER_WND: Balance = 1_00_000;

	/// The approximate number of WND for one US Dollar and so on...
	pub const MILLICENTS: Balance = UNITS_PER_WND / MILLICENTS_PER_WND;
