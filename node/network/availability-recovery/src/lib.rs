...
///
/// This (expensive) check is necessary, as otherwise we can't be sure that some chunk's won't have
/// been tampered with by the backers, which would result in some validators voting valid and some
/// voting invalid as having fetched different set of chunks. The checking of the Merkle proof for
/// individual chunks only gives us guarantees, that we have fetched a chunk belonging to a set the
/// backers have committed to.
///
...
/// data was invalid to begin with. In the former case, validators fetching valid chunks will vote
/// invalid as well, because the root won't match. In the latter case the situation is the same for
/// anyone anyways.
