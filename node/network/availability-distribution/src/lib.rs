...
use polkadot_primitives::v1::{
    AvailableData, BlakeTwo256, HashT, CommittedCandidateReceipt, CoreState, ErasureChunk,
    GlobalValidationData, Hash as H256, HeadData, Id as ParaId, LocalValidationData,
...
impl ProtocolState {
    fn add_relay_parent(&mut self, relay_parent: H256) -> &mut PerRelayParent {
        let prp = self.per_relay_parent.entry(relay_parent).or_default();
        prp
    }

    fn remove_relay_parent(&mut self, relay_parent: H256) {
        ...
        state.cached_relay_parents_to_live_candidates_unioned(added)
    {
        ...
        if let Some(pov) = query_proof_of_validity(ctx, candidate_hash.clone()).await? {
            ...
            if let Some(validator_index) = per_relay_parent.validator_index {
                let chunks: Vec<Vec<u8>> = if let Ok(chunks) =
                ...
                if let Some(erasure_chunk) = erasure_chunks.get(validator_index as usize) {
                    match store_chunk(ctx, candidate_hash, validator_index, erasure_chunk.clone())
                        .await
                    {
                ...
                for erasure_chunk in erasure_chunks {
                    ...
                    send_tracked_gossip_message_to_peers(ctx, per_relay_parent, peers, message)
                        .await?;
                }
            }
        }
    ...
    Context: SubsystemContext<Message = AvailabilityDistributionMessage>,
{
    ...
    let per_relay_parent =
        if let Some(ref mut per_relay_parent) = per_relay_parent {
            per_relay_parent
        } else {
