...
sp_api::mock_impl_runtime_apis! {
	impl ParachainHost<Block> for MockRuntimeApi {
		...
		fn check_validation_outputs(
			&self,
			para_id: ParaId,
			_commitments: polkadot_primitives::v3::CandidateCommitments,
		) -> bool {
	...
	impl BabeApi<Block> for MockRuntimeApi {
		...
		fn submit_report_equivocation_unsigned_extrinsic(
			_equivocation_proof: sp_consensus_babe::EquivocationProof<polkadot_primitives::v3::Header>,
...
fn requests_check_validation_outputs() {
	...
	let para_a = ParaId::from(5_u32);
	let para_b = ParaId::from(6_u32);
	let commitments = polkadot_primitives::v3::CandidateCommitments::default();
	let spawner = sp_core::testing::TaskExecutor::new();
