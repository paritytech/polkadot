...
lazy_static! {
	static ref BACKCHANNEL_HOST: String =
		env::var("BACKCHANNEL_HOST").unwrap_or_else(|_| "backchannel".to_string());
	static ref BACKCHANNEL_PORT: String =
...
impl ZombienetBackchannel {
	pub async fn connect() -> Result<ZombienetBackchannel, BackchannelError> {
		...
		Ok(ZombienetBackchannel { inner: read })
	}
	...
	pub async fn send(key: &'static str, val: impl codec::Encode) -> Result<(), BackchannelError> {
		...
		let zombienet_endpoint =
			format!("http://{}:{}/{}", *BACKCHANNEL_HOST, *BACKCHANNEL_PORT, key);

		tracing::debug!(target = ZOMBIENET, "Sending to: {}", &zombienet_endpoint);
		let _res =
			client
				.post(zombienet_endpoint)
				.body(body.to_string())
				.send()
				.await
				.map_err(|e| {
