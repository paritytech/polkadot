...
pub struct OverseerArgs<'a, Spawner, RuntimeClient> where
...
/// Alias for the fn generating the overseer.
pub type OverseerGenFn<Spawner, RuntimeClient>
	= dyn for<'a>
		Fn(OverseerArgs<'a, Spawner, RuntimeClient>)
			-> Result<(Overseer<Spawner, Arc<RuntimeClient>>, OverseerHandler), Error>;

pub fn real_overseer<'a, Spawner, RuntimeClient>(
	OverseerArgs {
		leaves,
		keystore,
		runtime_client,
	...
	} : OverseerArgs<'a, Spawner, RuntimeClient>
) -> Result<(Overseer<Spawner, Arc<RuntimeClient>>, OverseerHandler), Error>
where
	RuntimeClient: 'static + ProvideRuntimeApi<Block> + HeaderBackend<Block> + AuxStore,
	RuntimeClient::Api: ParachainHost<Block> + BabeApi<Block> + AuthorityDiscoveryApi<Block>,
