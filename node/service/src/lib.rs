...
pub fn new_light<Runtime, Dispatch>(mut config: Configuration) -> Result<(
	TaskManager, Arc<LightClient<Runtime, Dispatch>>, Arc<LightBackend>, NetworkStatusSinks<Block>,
), Error>
	where
...
pub fn new_light_chain_ops<Runtime, Dispatch>(config: Configuration) -> Result<(
	...
	where
		...
		Dispatch: NativeExecutionDispatch + 'static,
{
	let (task_manager, client, backend, network) = new_light(config)?;
...
{
	let (task_manager, ..) = new_light::<polkadot_runtime::RuntimeApi, PolkadotExecutor>(config)?;
	Ok(task_manager)
}
...
{
	let (task_manager, ..) = new_light::<kusama_runtime::RuntimeApi, KusamaExecutor>(config)?;
	Ok(task_manager)
}
...
{
	let (task_manager, ..) = new_light::<westend_runtime::RuntimeApi, KusamaExecutor>(config)?;
	Ok(task_manager)
}
