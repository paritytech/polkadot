...
fn prepare_enum_variant(variant: &mut Variant) -> Result<EnumVariantDispatch> {
	...
	let inner = match variant.fields.clone() {
		...
		Fields::Named(FieldsNamed { brace_token: _, named }) if !skip => named
			...
			.ok_or_else(|| Error::new(span, "To dispatch with struct enum variant, one element must named `inner`"))?,
		...
		_ if skip => None,
		Fields::Unit => return Err(Error::new(span, "Must be annotated with #[skip].")),
		Fields::Unnamed(_) => {
...
fn impl_wrapper_dispatch_fn(name: Ident, fields: &[SubSysField]) -> Result<proc_macro2::TokenStream> {
