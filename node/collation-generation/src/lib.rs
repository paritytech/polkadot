		...
		Context: overseer::SubsystemContext<Message = CollationGenerationMessage>,
	{
		match incoming {
			Ok(FromOverseer::Signal(OverseerSignal::ActiveLeaves(ActiveLeavesUpdate {
				...
				if let Some((config, activated)) = self.config.as_ref().zip(activated) {
					let metrics = self.metrics.clone();
					if let Err(err) =
						handle_new_activation(config.clone(), activated.hash, ctx, metrics, sender)
							.await
					{
...
async fn handle_new_activation<Context: SubsystemContext>(
	...
	for (core_idx, core) in availability_cores.into_iter().enumerate() {
		...
		ctx.spawn(
			...
			Box::pin(async move {
				...
				let pov = {
					let pov =
						polkadot_node_primitives::maybe_compress_pov(collation.proof_of_validity);
			...
			new_activations_per_relay_parent: prometheus::register(
				prometheus::Histogram::with_opts(
					prometheus::HistogramOpts::new(
						...
						"Time spent handling a particular relay parent within fn handle_new_activation"
					)
				)?,
				registry,
			)?,
			new_activations_per_availability_core: prometheus::register(
				prometheus::Histogram::with_opts(
					prometheus::HistogramOpts::new(
						...
						"Time spent handling a particular availability core for a relay parent in fn handle_new_activation",
					)
				)?,
				registry,
			)?,
