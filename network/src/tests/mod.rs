...
fn sends_session_key() {
	...
	{
		let mut ctx = TestContext::default();
		let params = make_validation_session(parent_hash, local_key);
...
fn many_session_keys() {
	...
	let params_a = make_validation_session(parent_a, local_key_a);
	let params_b = make_validation_session(parent_b, local_key_b);
