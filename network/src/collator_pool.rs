...
use polkadot_primitives::parachain::{Id as ParaId, CollatorId, Collation};
...
impl CollatorPool {
	...
	pub fn on_new_collator(&mut self, account_id: CollatorId, para_id: ParaId) -> Role {
	...
	/// parachain.
	pub fn on_disconnect(&mut self, account_id: CollatorId) -> Option<CollatorId> {
	...
	pub fn on_collation(&mut self, account_id: CollatorId, relay_parent: Hash, collation: Collation) {
...
mod tests {
	...
	fn disconnect_primary_gives_new_primary() {
		...
		assert_eq!(pool.on_disconnect(bad_primary.clone()), Some(good_backup.clone()));
	...
	fn disconnect_backup_removes_from_pool() {
		...
		assert_eq!(pool.on_disconnect(backup.clone()), None);
